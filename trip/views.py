...
class TripCrateView(CreateView):
    model = Trip
    success_url = reverse_lazy('trip-list')
    fields = [ 'city','country','start_date', 'end_date']
    # will be looking for template named model_form.html => trip.form.html
...
class TripDetailView(DetailView):
    model = Trip
    # we want data stored on trip - also have the data of the notes
...
class NoteDeleteView(DeleteView):
    model = Note
    success_url = reverse_lazy('note-list')
    # no templete needed - send a post request to this url
...
class TripDeleteView(DeleteView):
    model = Trip
    success_url = reverse_lazy('trip-list')
    # no template needed - send post request to this url
